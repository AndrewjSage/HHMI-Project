#Used to create the table with at-risk status and "error rates."
...
STEM$Class=as.numeric(STEM$Class == 2 | STEM$Class == 1)
...
##Here we consider a binary response of leaving (which could mean leaving STEM or ISU). Used for 
##confusion table in paper. 
...
Result <- data.frame(CFPred, Truth) %>% arrange(desc(CFPred))
Result$CumSumLeft <- cumsum(Result$Truth)
Result$PropLeft <- Result$CumSumLeft / (1:nrow(Result))
Result[379,] # if 10% were tagged
Result[567,] # if 15% were tagged
Result[758,] # if 20% were tagged
