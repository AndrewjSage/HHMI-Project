...
#Students1=data.frame(Students$actCmpst, Students$hsRank, Students$hsGpa)
#Students1=mice(Students1, m = 1, method = vector("character", length = ncol(Students1)),
#               predictorMatrix = (1 - diag(1, ncol(Students1))),
#               visitSequence = (1:ncol(Students1))[apply(is.na(Students1), 2, any)],
#               form = vector("character", length = ncol(Students1)),
#               post = vector("character", length = ncol(Students1)), defaultMethod = c("pmm",
...
#Students1=complete(Students1, 1)
#HSGrades=(scale(Students1$Students.hsRank)+scale(Students1$Students.hsGpa))/2
...
setwd("~/Box Sync/Iowa State/Engage Analysis/Current Code/Spring 2017")
